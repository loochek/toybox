...
PluginPicker::PluginPicker(const Vec2i &position, PPluginType pickerType, Widget *parent) :
...
{
    assert(plugin->getInfo()->type == mType);
    ...
    pluginButton->setLabel(plugin->getInfo()->name);
